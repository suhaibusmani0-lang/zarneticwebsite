            ...
            <div className="flex flex-col gap-10">
              <div className="relative group w-fit cursor-pointer">
                ...
                <div className="relative bg-white rounded-2xl p-2 shadow-xl transition-transform group-hover:scale-105">
                  <img src={logoSrc} alt="Zarnetic Logo" className="w-64 h-auto object-contain" />
                </div>
              </div>

              <div className="space-y-6 text-gray-400 font-light">
                <div className="flex items-start gap-4 group">
                  <div className="mt-1.5 w-8 h-8 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center shrink-0 group-hover:border-red-500/50 transition-colors" aria-hidden="true">
                  ...
                  <address className="not-italic leading-relaxed text-sm lg:text-base">
                    ...
                    <span>36 1st Floor, Lane No. 7, Noor Nagar, Jamia Nagar,<br />Okhla, New Delhi – 110025</span>
                  </address>
                </div>

                <div className="flex items-center gap-4 group">
                  <div className="w-8 h-8 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center shrink-0 group-hover:border-emerald-500/50 transition-colors" aria-hidden="true">
              ...
              <div className="flex items-center flex-wrap gap-4 pt-4">
                {[
                  { name: 'Facebook', link: 'https://www.facebook.com/share/1AvVWEx2Ei/', color: 'hover:bg-[#1877F2]', path: "M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" },
                  { name: 'Instagram', link: 'https://www.instagram.com/zarnetic?igsh=MTkwbjVwcGNsanhpNA==', isInsta: true, color: 'hover:bg-white/10' },
                  ...
                  >
                    <svg width="18" height="18" viewBox="0 0 24 24" aria-hidden="true" className={`relative z-10 transition-transform duration-500 group-hover/social:scale-110 ${social.isMail ? 'text-white group-hover/social:text-black' : 'fill-white'}`}>
                      {social.isInsta ? (
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-y-10 gap-x-6">
                  {techStack.map((tool) => (
                    <div key={tool} className="group/item flex items-center gap-3">
                      ...
                      <span className="text-gray-400 group-hover/item:text-white transition-colors duration-300 font-medium text-sm">{tool}</span>
                    </div>
                  ))}
                </div>
              </div>
